...
func (l *PublishLogic) Publish(title, description string, file multipart.File, header *multipart.FileHeader) (resp *types.BaseResp, err error) {
	authUser, err := l.svcCtx.App.MustAuth(l.ctx)
	if err != nil {
		return nil, err
	}
	if err := l.svcCtx.App.PublishVideo(l.ctx, authUser.UserID, title, description, file, header); err != nil {
		return nil, err
	}
